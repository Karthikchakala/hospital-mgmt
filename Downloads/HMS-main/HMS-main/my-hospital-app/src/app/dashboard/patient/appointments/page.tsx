...
// CRITICAL: We need the Razorpay global object (script loaded via index.html or <Script>)
declare global {
    interface Window {
        Razorpay: any;
    }
...
export default function AppointmentsPage() {
    ...
    const [isLoading, setIsLoading] = useState(true);
    const [userName, setUserName] = useState('');
    const [userEmail, setUserEmail] = useState(''); // Store user email for payment prefill
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setUserName(userResponse.data.name);
                setUserEmail(userResponse.data.email); // Store email for payment
                setDepartments(deptResponse.data);
            } catch (error) {
                console.error('Failed to fetch initial data:', error);
                localStorage.removeItem('token');
    ...
    // --- 2. Fetch Doctors (Cascading Logic) ---
    useEffect(() => {
        // ... (Existing doctor fetching logic remains the same)
        if (selectedDepartment) {
            const fetchDoctors = async () => {
                const token = localStorage.getItem('token');
                if (!token) return;
    ...
    // --- 3. Fetch Time Slots ---
    useEffect(() => {
        // ... (Existing time slot fetching logic remains the same)
        if (selectedDoctor && selectedDate) {
            const fetchTimeSlots = async () => {
                const token = localStorage.getItem('token');
                if (!token) return;
    ...
    // --- Payment Success Handler (Final Booking After Transaction) ---
    const finalizeAppointmentAndBill = async (paymentResponse: any, orderId: string, amount: number) => {
        const token = localStorage.getItem('token');
        if (!token) return;

        try {
            // CRITICAL: This is a placeholder for your backend logic to finalize the bill and appointment
            // The existing /api/patient/appointments route needs to be updated on the backend to handle the payment payload
            await axios.post(
                'http://localhost:5000/api/patient/appointments', 
                {
                    doctorId: selectedDoctor,
                    ...
                    appointmentTime: selectedTime,
                    reason: reason,
                    // Pass payment details to trigger bill creation and status update on backend
                    paymentDetails: {
                        orderId: orderId,
            ...
            alert('✅ Appointment booked and paid successfully! Check your bills.');
            router.push('/dashboard/patient/bills'); // Redirect to bills page
            
        } catch (error) {
            console.error('Appointment Finalization Failed:', error);
            alert('Appointment booked but failed to finalize payment status. Please contact support.');
        }
    };


    // --- Payment Initiation and Checkout Modal ---
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const token = localStorage.getItem('token');
        ...
        try {
            // STEP 1: Call backend to generate Order ID
            const txnResponse = await axios.post('http://localhost:5000/api/patient/payment/generate-txn', {
                // billId: 0, // Placeholder, actual bill created on backend
                // doctorId: selectedDoctor,
                amount: price,
            }, { headers: { Authorization: `Bearer ${token}` } });
            ...
            const options = {
                ...
                description: `Appointment Fee for Dr. ${doctors.find(d => String(d.doctor_id) === selectedDoctor)?.name || ''}`,
                order_id: orderId, 
                handler: async (response: any) => {
                    // STEP 3: Handle successful payment response
                    await finalizeAppointmentAndBill(response, orderId, price);
                },
                prefill: {
                    name: userName, 
                    email: userEmail,
                    contact: '9876543210' // Placeholder contact
                },
                theme: { color: '#3B82F6' }
            };
            ...
            alert(`Payment initiation failed: ${error.response?.data?.message || 'Check backend logs.'}`);
        }
    };

    // --- Unified Change Handlers (remain the same) ---
    const handleDepartmentChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        setSelectedDepartment(e.target.value);
        setSelectedDoctor('');
        setSelectedDate('');
                ...
                <div className="bg-white p-8 rounded-lg shadow-md max-w-2xl mx-auto">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <button
                            ...
                            disabled={!selectedDoctor || !selectedDate || !selectedTime || !reason || price <= 0}
                        >
