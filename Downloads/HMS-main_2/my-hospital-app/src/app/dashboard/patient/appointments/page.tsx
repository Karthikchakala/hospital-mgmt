...
import axios from 'axios';
import { useRouter } from 'next/navigation';
import ParticlesBackground from '../../../../components/ParticlesBackground'; // 🔹 Reuse the animated background
...
export default function AppointmentsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-sky-950 via-slate-900 to-teal-950 text-white">
        <h1 className="text-2xl font-bold">Loading...</h1>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen font-sans text-gray-100 bg-gradient-to-br from-sky-950 via-slate-900 to-teal-950 overflow-hidden">
      <ParticlesBackground /> {/* 🔹 adds subtle animated background behind everything */}
      <DashboardNavbar
        title="Patient Portal"
        navLinks={patientNavLinks}
        userName={userName}
      />
      <main className="container mx-auto py-12 px-6 relative z-10">
        <h1 className="text-4xl font-bold mb-8 text-cyan-300">Appointments</h1>
        <div className="bg-slate-900/80 border border-cyan-800 p-8 rounded-xl shadow-lg max-w-2xl mx-auto backdrop-blur-md">
          <h2 className="text-2xl font-semibold mb-6 text-white">Book a New Appointment</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Department Selection */}
            <div>
              ...
              <select
                ...
                className="w-full p-3 rounded-md bg-gray-800/70 text-white border border-gray-600 focus:ring-2 focus:ring-cyan-400"
              >
                <option value="">-- Choose a Department --</option>
                {departments.map((dept) => (
                  <option key={dept.department_id} value={dept.department_id}>
            ...
            <div>
              ...
              <select
                ...
                disabled={!selectedDepartment || doctors.length === 0}
                className="w-full p-3 rounded-md bg-gray-800/70 text-white border border-gray-600 focus:ring-2 focus:ring-cyan-400 disabled:opacity-50"
              >
                <option value="">-- Choose a Doctor --</option>
                {doctors.map((doctor) => (
                  <option key={doctor.doctor_id} value={doctor.doctor_id}>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 border rounded-md transition-all duration-300 ${
                        selectedTime === time
                          ...
                          : 'bg-gray-800/70 text-gray-200 hover:bg-gray-700 border-gray-600'
                      }`}
                    >
                      {time}
                    </button>
